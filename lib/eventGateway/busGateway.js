    ...
    return function (req, res, next) {
        ...
        req.on('end', function () {
            try {
                ...
                debug_log('Error in busGateway dispatch: ' + e);
...
BusGateway.prototype._uplinkRecv = function (jsonObj, req, res) {
    ...
    if (!zone) {
        ...
        setTimeout(function () {
            ...
            if (!zone) {
                debug_log('gateway not found');
                res.send(200, 'Gateway not found');
            } else {
                debug_log('retry _uplinkRecv');
                self._uplinkRecv(jsonObj, req, res);
            }
        }, AJAX_TIMEOUT / 10);
        return;
...
BusGateway.prototype._downlinkRecv = function (jsonObj, req, res) {
    ...
    if (!zone) {
        ...
        debug_log('online: ' + jsonObj.meta[META_FROM]);
    } else {
        zone.req = req;
        zone.res = res;
        zone.sending = false;
